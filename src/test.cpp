...
static const char kUsage[] = "Usage: example program to load data";

static bool ValidateFilename(const char*, const std::string& value) {
  return value.size() > 0;
}

static bool ValidateLambda(const char*, const double lambda) {
  return lambda >= 0.0;
}

DEFINE_string(factory_world,
              "",  // no default
              "the file containing information about the world of factory");
DEFINE_double(lambda, 0.01, "The weight of l1 norm");
DEFINE_double(time_limit, 10, "Time limit for the solver(seconds)");
DEFINE_string(output_file, "output_schedule", "The scheduler's output");
...
int main(int argc, char** argv) {
  ...
  planner.factoryScheduler(
      ...
      operations_research::MPSolver::
          CBC_MIXED_INTEGER_PROGRAMMING,  // GLOP_LINEAR_PROGRAMMING);
      FLAGS_lambda,
      FLAGS_time_limit * 1000, outputStream);
